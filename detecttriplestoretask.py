    ...
    def detectTripleStoreConfiguration(self):	
        ...
        if self.testTripleStoreConnection(testQueries["available"]):
            if self.testTripleStoreConnection(testQueries["hasWKT"]):
                ...
                self.configuration["featurecollectionclasses"]=["http://www.opengis.net/ont/geosparql#FeatureCollection"]
                self.configuration["geometrycollectionclasses"]=["http://www.opengis.net/ont/geosparql#GeometryCollection"]
                ...
                self.configuration["geocollectionquery"]="SELECT DISTINCT ?colinstance ?label  WHERE { ?colinstance rdf:type %%concept%% . OPTIONAL { ?colinstance rdfs:label ?label . } }"
