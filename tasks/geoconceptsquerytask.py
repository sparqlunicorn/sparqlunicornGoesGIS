...
class GeoConceptsQueryTask(QgsTask):
    ...
    def run(self):
        if os.path.exists(os.path.join(__location__,"../tmp/geoconcepts/" + str(self.triplestoreconf["resource"]["url"].replace("/", "_").replace("\\","_").replace(":","_")) + ".json")):
    ...
    def finished(self, result):
        ...
        if self.loadfromfile:
            elemcount=UIUtils.loadTreeFromJSONFile(self.rootNode,os.path.join(__location__,
                         "../tmp/geoconcepts/" + str(self.triplestoreconf["resource"]["url"].replace("/", "_").replace("\\","_").replace(":","_")) + ".json"))
            ...
            f = open(os.path.join(__location__, "../tmp/geoconcepts/" + str(
                self.triplestoreconf["resource"]["url"].replace("/", "_").replace("\\","_").replace(":", "_")) + ".json"), "w")
            res = {"text": "root"}
