...
class QuerySubGraphTask(QgsTask):
    ...
    def __init__(self, description,concept, triplestoreurl, query, triplestoreconf, progress=None,querydepth=0):
        ...
        self.exception=None
        self.exportToFunction = {"CYPHER":GraphExporter.convertTTLToCypher,"GRAPHML": GraphExporter.convertTTLToGraphML, "GDF": GraphExporter.convertTTLToTGF, "TGF": GraphExporter.convertTTLToTGF,
    ...
    def finished(self, result):
        ...
        if self.exception!=None:
            ...
            return
        filename = QFileDialog().getSaveFileName(None,"Save TTL result",self.concept+".ttl","Linked Data (*.n3 *.nt *.trig *.ttl *.xml) Graph Data (*.gdf *.graphml *.tgf)")
        QgsMessageLog.logMessage(str(self.results),
                                 MESSAGE_CATEGORY, Qgis.Info)
        if filename:
