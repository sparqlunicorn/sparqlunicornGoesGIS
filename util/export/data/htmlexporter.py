...
from ..api.iiifexporter import IIIFAPIExporter
from ..pages.geometryviewpage import GeometryViewPage
from ..pages.lexiconpage import LexiconPage
from ..pages.personpage import PersonPage
from ..pages.model3dpage import Model3DPage

from ..pages.textannopage import TextAnnoPage
...
class HTMLExporter:
    ...
    def createHTML(self, savepath, predobjs, subject, baseurl, subpreds, graph, searchfilename, classtreename,
                ...
                if predobjtuplen > 0:
                    ...
                    for item in predobjmap[tup]:
                        ...
                        if tup in DocConfig.valueproperties and (
                                "POINT" in str(item).upper() or "POLYGON" in str(item).upper() or "LINESTRING" in str(
        ...
        with open(completesavepath, 'w', encoding='utf-8') as f:
            ...
            f.write(DocUtils.replaceStandardVariables(self.templates["htmltemplate"], subject, checkdepth, "false",
                                                      self.pubconfig).replace(
                "{{iconprefixx}}", (relpath + "icons/" if self.pubconfig["offlinecompat"] else "")).replace(
                ...
                "{{toptitle}}", foundlabel).replace("{{startscriptpath}}", startscriptlink).replace("{{epsgdefspath}}",
                                                                                                    epsgdefslink).replace(
                "{{bibtex}}", itembibtex).replace("{{vowlpath}}", vowlresultlink).replace("{{proprelationpath}}",
                                                                                          proprelationslink).replace(
                "{{stylepath}}", csslink).replace("{{title}}", "<a href=\"" + str(subject) + "\">" + str(
            ...
            if len(foundmedia["mesh"]) > 0:
                self.has3d = Model3DPage.generatePageWidget(graph, foundmedia, annobodies, self.templates, subject,
                                                            self.iiifmanifestpaths, image3dannos, self.pubconfig,
                                                            self.imagetoURI, foundlabel, comment, thetypes, predobjmap,
                                                            f, checkdepth)
            ...
            if len(imageannos) > 0 and len(foundmedia["image"]) > 0:
                if self.pubconfig["apis"]["iiif"]:
                    self.iiifmanifestpaths["default"].append(
                        IIIFAPIExporter.generateIIIFManifest(graph, self.pubconfig["outpath"],
                                                             self.pubconfig["deploypath"],
                                                             foundmedia["image"], imageannos, annobodies,
                                                             str(subject), self.pubconfig["prefixns"], self.imagetoURI,
                                                             self.pubconfig["imagemetadata"],
                                                             DocConfig.metadatanamespaces,
            ...
            tempfoot = DocUtils.replaceStandardVariables(self.templates["footer"], "", checkdepth, "false",
                                                         self.pubconfig).replace(
                "{{exports}}",
                myexports).replace(
            ...
            tempfoot = DocUtils.conditionalArrayReplace(tempfoot,
                                                        [True, self.pubconfig["apis"]["ogcapifeatures"],
                                                         self.pubconfig["apis"]["iiif"],
            ...
            if tuppredstr in DocConfig.labelproperties:
                ...
                if pred == "http://www.w3.org/ns/oa#hasSelector":
                    if tupobjstr == "http://www.w3.org/ns/oa#SvgSelector" or tupobjstr == "http://www.w3.org/ns/oa#WKTSelector":
                        ...
                        "http://purl.org/ontology/bibo/" in tupobjstr):
                ...
                if not inverse and (
                object, typeprop, URIRef("http://www.w3.org/ns/sosa/ObservationCollection")) in graph:
        ...
        if foundval is not None:
            if foundunit is not None:
                ...
                if pred == "http://www.w3.org/ns/oa#hasBody":
                    ...
                    annobodies.append(
                        {"value": foundval, "unit": foundunit, "type": "TextualBody", "format": "text/plain"})
            else:
    ...
    def createHTMLTableValueEntry(subject, pred, object, ttlf, graph, baseurl, checkdepth, geojsonrep, foundmedia,
        ...
        if isinstance(object, URIRef) or isinstance(object, BNode):
            ...
            unitlabel = ""
            mydata = HTMLExporter.searchObjectConnectionsForAggregateData(graph, object, predstr, geojsonrep,
                                                                          foundmedia,
            ...
            if isinstance(object, Literal) and object.datatype is not None:
                ...
                if object.datatype == XSD.anyURI:
                    objstring = f"<a href=\"{objstr}\">{objstr}</a>"
                        ...
                        object.datatype) in DocConfig.timeliteraltypes and dateprops is not None and DocUtils.shortenURI(
                        predstr, True) not in DocConfig.metadatanamespaces and str(pred) not in dateprops:
                    dateprops.append(predstr)
                tablecontents += f"<span itemprop=\"{predstr}\" property=\"{predstr}\" content=\"" + objstr.replace("<",
                                                                                                                    "&lt").replace(
                    ">", "&gt;").replace("\"",
                                         "'") + f"\" datatype=\"{object.datatype}\">{HTMLExporter.truncateValue(objstring)} <small>(<a style=\"color: #666;\" target=\"_blank\" href=\"" + str(
    ...
    def formatPredicate(tup, baseurl, checkdepth, tablecontents, graph, reverse, labellang, prefixes):
        ...
        if baseurl in str(tup):
            ...
            tablecontents += f"<span class=\"property-name\"><a class=\"uri\" target=\"_blank\" href=\"{tup}\">{label} " + (
                "<span style=\"color: #666;\">(" + res["uri"] + ")</span>" if res["uri"] != "" else "") + "</a> </span>"
        if reverse:
            tablecontents += " of"
        # tablecontents += "</td>"
    ...
    def detectStringLiteralContent(pred, object):
        ...
        return f"<span property=\"{pred}\" content=\"" + str(object).replace("<", "&lt").replace(">", "&gt;").replace(
